...
SITE_NAME="${SITE_NAME:-AdDevSelfHostedSite}"
DOMAIN="${DOMAIN:-www.addevselfhostedsited.com}"
SITE_DIR="${SITE_DIR:-/var/www/addevselfhostedsited}"
NGINX_CONF="/etc/nginx/sites-available/addevselfhostedsited"
NGINX_ENABLED="/etc/nginx/sites-enabled/addevselfhostedsited"
LOG_FILE="${LOG_FILE:-/var/log/site_deployment.log}"
BACKUP_DIR="/tmp/site_backup_$(date +%Y%m%d_%H%M%S)"
...
show_help() {
    cat << EOF
AdDevSelfHostedSite Deployment Script

USAGE:
    $0 [OPTIONS]

OPTIONS:
    -d, --domain DOMAIN     Set custom domain (default: www.addevselfhostedsited.com)
    -p, --path PATH         Set custom installation path (default: /var/www/addevselfhostedsited)
    -s, --ssl               Enable SSL/HTTPS setup with Let's Encrypt
    -b, --backup            Create backup of existing installation
    -r, --restore PATH      Restore from backup path
    -u, --uninstall         Uninstall the site
    -h, --help              Show this help message
    --dry-run               Show what would be done without executing
    --force                 Force installation even if site exists

EXAMPLES:
    $0                                          # Default installation
    $0 -d example.com -p /var/www/example       # Custom domain and path
    $0 -s                                       # Install with SSL
    $0 --backup                                 # Create backup before install
    
ENVIRONMENT VARIABLES:
    DOMAIN, SITE_DIR, LOG_FILE, SSL_ENABLED can be set to override defaults
...
install_packages() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        log "[DRY RUN] Would install packages based on $PKG_MANAGER"
        return
    fi
    
    if [[ "$PKG_MANAGER" == "apt" ]]; then
        apt update -qq
        DEBIAN_FRONTEND=noninteractive apt install -y nginx ufw curl wget unzip
        if [[ "$SSL_ENABLED" == "true" ]]; then
            DEBIAN_FRONTEND=noninteractive apt install -y certbot python3-certbot-nginx
        fi
    elif [[ "$PKG_MANAGER" == "yum" ]]; then
        yum update -y -q
        yum install -y nginx firewalld curl wget unzip
        if [[ "$SSL_ENABLED" == "true" ]]; then
...
create_backup() {
    if [[ "$CREATE_BACKUP" == "true" ]] && [[ -d "$SITE_DIR" ]]; then
        log "Creating backup..."
...
restore_backup() {
    if [[ -n "$RESTORE_PATH" ]]; then
        ...
        if [[ ! -d "$RESTORE_PATH" ]]; then
            error "Backup path does not exist: $RESTORE_PATH"
        fi
        ...
        cp -r "$RESTORE_PATH/site" "$SITE_DIR"
        if [[ -f "$RESTORE_PATH/nginx.conf" ]]; then
            cp "$RESTORE_PATH/nginx.conf" "$NGINX_CONF"
...
uninstall_site() {
    if [[ "$UNINSTALL" == "true" ]]; then
        log "Uninstalling site..."
        
        if [[ "$DRY_RUN" == "true" ]]; then
            log "[DRY RUN] Would remove $SITE_DIR and nginx config"
            return
        fi
        ...
        success "Site uninstalled successfully"
        exit 0
    fi
}
